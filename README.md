# FinSage

**FinSage** is a financial analysis skill designed to help investors analyze securities through a **portfolio manager’s perspective**.  
By integrating plugins and external data or workflow tools, FinSage aims to deliver more structured, research-oriented, and decision-supportive analysis for investment use cases.

## Description

FinSage is built for investors who want to go beyond surface-level market commentary and evaluate securities with a more disciplined, portfolio-manager-style lens.

Instead of treating investment analysis as isolated stock picking, FinSage is designed to support a broader analytical workflow, including:

- security-level analysis
- portfolio context awareness
- risk and return framing
- comparative evaluation
- plugin-based extension for specialized tools and data sources

The project can be used as a skill layer inside AI-assisted workflows, enabling users to combine language-model reasoning with plugin-powered financial analysis capabilities.

## Core Idea

FinSage helps users think more like a professional portfolio manager by emphasizing:

- **Thesis-driven analysis**  
  Understand what makes a security attractive or unattractive.

- **Risk-aware evaluation**  
  Consider downside, uncertainty, concentration risk, and scenario sensitivity.

- **Portfolio perspective**  
  Analyze a security not only on its own merits, but also in relation to diversification, allocation, and fit within a broader portfolio.

- **Plugin extensibility**  
  Connect external plugins, tools, or data pipelines to enrich analysis and workflows.

## Potential Use Cases

FinSage may support workflows such as:

- equity research assistance
- security screening and comparison
- portfolio review and discussion support
- investment memo drafting
- risk factor mapping
- scenario and sensitivity analysis
- integrating external financial data plugins into AI workflows

## Design Principles

- **Investor-oriented**: built for real-world investment thinking, not generic summaries
- **Portfolio-manager mindset**: emphasizes judgment, trade-offs, and context
- **Plugin-friendly**: designed to work with external capabilities and data connectors
- **Explainable workflows**: aims to produce reasoning that is structured and reviewable
- **Modular**: can evolve with different tools, strategies, and research needs

## Important Notice

FinSage uses AI to support financial analysis workflows.  
Like all AI-based tools, its outputs may be incomplete, inaccurate, outdated, or not fully aligned with a user’s specific financial circumstances, objectives, or risk tolerance.

**FinSage is for informational and research support purposes only and does not provide personalized investment advice, legal advice, tax advice, or any other regulated professional advice.**

Users should not rely solely on AI-generated analysis when making investment decisions.  
Before making any investment decision, users are strongly encouraged to consult a **licensed and registered investment advisor** or another qualified financial professional in their jurisdiction.

## Project Status

FinSage is an evolving project.  
Its architecture, plugin integrations, and analytical workflows may continue to expand over time.

## License

This project is licensed under the **MIT License**.  
See the [LICENSE](LICENSE) file for details.
